...
sub _build__connection {
    ...
    $self->{dbx} = MongoDBx::Class->new(namespace => 'Slaver::Schema', search_dirs => ['lib/Slaver/Schema']) unless $self->{dbx};
...
sub get_session_data {
	...
	if ($found->{expires} && time() > $found->{expires}) {
		...
		my $issue_datetime = DateTime->now;
		$self->_collection->update({ _id => $id }, { '$set' => { issue => $issue_datetime } });
	}
...
sub store_session_data {
	...
	$self->_collection->update({ _id => $id },
		{ '$set' => { $prefix => $serialized, issue => $issue_datetime } }, { upsert => 1 });
}
